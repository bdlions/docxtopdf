...
class RiskRegisterPDF extends FPDF {
	...
	//$this->SetDrawColor(128,0,0);
        ...
        foreach ($this->risk_register_rows as $value) {
            ...
            //adding a row
            $h = $this->getFillHeight($col_data);
            $this->SetXY($x, $y);
            
            $this->fillBackGroundColor($col_data, $h);
    ...
    /*function addRow($col_data) {
        ...
        for ($col = 0; $col < count($col_data); $col++) {
            $this->Cell($col_data[$col]['cell_width'],$maxRowHeight,'',"LRTB",0,'C',true);
    ...
    function fillBackGroundColor($col_data, $height){
        ...
        function getFillHeight($col_data) {
